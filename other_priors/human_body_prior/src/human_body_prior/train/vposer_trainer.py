...
class VPoserTrainer(LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.renderer is not None and self.global_rank == 0 and batch_idx % 500 == 0 and np.random.rand() > 0.5:
            out_fname = makepath(self.work_dir,
                                 'renders/valid_rec_E{:03d}_It{:04d}_val_loss_{:.2f}.png'.format(self.current_epoch,
                                                                                                 batch_idx,
                                                                                                 val_loss.item()),
                                 isfile=True)
            ...
            out_fname = makepath(self.work_dir,
                                 'renders/valid_gen_E{:03d}_I{:04d}.png'.format(self.current_epoch, batch_idx),
                                 isfile=True)
    ...
    def on_validation_epoch_end(self):
        if self.global_rank == 0:
            current_lr = [pg['lr'] for opt in self.trainer.optimizers for pg in opt.param_groups]
            self.text_logger(
                f'Epoch {self.current_epoch}: val_loss: {self.trainer.callback_metrics["val_loss"].item():.2f}, lr: {current_lr}')
